...
- `r4300/compare_core.c:79-111` -- gated behind `COMPARE_CORE`, not defined anywhere, currently dead (and has a latent `fread` size bug if ever revived: `:88,92,96` read `4*sizeof(long)` bytes where 4 is wanted). Left alone: unlike `LAZY_GEN_CALLS`, `COMPARE_CORE` also gates code in `Wrappers.c`/`Wrappers.h`/`r4300.c`, a debug-comparison subsystem rather than an isolated unused code path -- removing it is a bigger, riskier edit than this pass's other dead-code deletions for a "very low priority", currently-harmless finding.
