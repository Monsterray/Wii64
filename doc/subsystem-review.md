...
- `r4300/ppc/Wrappers.c` (`dyna_run`/`RecompCache_Link`, cross-referenced with `Recomp-Cache-Heap.c`'s `RecompCache_Link`) -- **found the concrete mechanism the CIC-6105 hang uses to evade both `dynarec_trace` and the `DYNAREC_WATCHDOG_CYCLE_LIMIT` safety net.** `RecompCache_Link()` patches a block's exit branch into a direct native `B` (unconditional jump, no link) straight to another block's entry point, replacing the `blrl` that would otherwise return control to `dynarec()`'s C loop (see `genJumpTo()`, `MIPS-to-PPC.c:4140-4168`: the linkable exit path stores `last_pc` then does `GEN_BLR(1)`, which `RecompCache_Link` can overwrite with a direct branch). Two or more blocks linked into a cycle can then run forever in pure native PPC code without ever returning to the outer `while(!r4300.stop)` loop -- which is exactly where `dynarecTrace_dispatch()` and the cycle-limit watchdog both live. This matches the Majora's Mask repro precisely: the dispatch count freezes (dynarec_trace showed an unchanged trace, stuck at dispatch #768) while the guest exception count keeps climbing (a genuine C function call to `exception_general()` from within the native loop still runs and still counts, even though it never returns control to the C dispatcher). The one designed escape hatch, `GEN_BGELR(CR3,0)` (return early on a pending interrupt/timer event, checked before the linkable exit), evidently isn't reached on this loop's particular path. **Not fixed this pass** -- this is now a precise, evidence-backed lead rather than a guess, but confirming *why* the escape hatch is skipped needs either a disassembly of the actual linked blocks or new instrumentation inside the native trampoline itself, both real investigative work, not a quick patch. See `doc/reference-projects.md` for the earlier (unsuccessful) `pif.c` fix attempt this supersedes as the leading theory.
