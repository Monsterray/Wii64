...
`fileBrowser-DVD.c:118` (`strcpy(direntry->name, DVDToc[i].name)`) had the
...
**Cleanup**:
- `rsp_hle/plugin.c` set `g_hle.hle_aud`/`hle_gfx` unconditionally (0 and 1) and never toggled either -- the branches and dead `send_alist_to_audio_plugin()` they guarded were permanent vestiges of upstream's configurable HLE/LLE split, unused since only one of Rice_GX/glN64_GX is ever linked in. **APPLIED**: both fields and the branches/function they guarded removed.
...
- `Rice_GX/Config.cpp:2100-3562` (~1460 lines) -- the entire GTK desktop config-dialog, `#ifndef __GX__`, permanently dead. **APPLIED** (deleted; ran to EOF).
...
**Possible lead #2, APPLIED**: `gc_memory/dma.c` (`dma_sp_write`/`dma_sp_read`, RSP<->RDRAM DMA used by every audio/graphics ucode task) masked `dramaddr` once at entry but never re-clamped it per loop iteration against actual `rdram[]` size, so a `count`/`skip` combination could walk it out of bounds mid-loop. Fixed: re-mask inside the loop.

**Possible lead #3, APPLIED**: `gc_memory/pif.c`'s `internal_ControllerCommand()` mempak read/write touched up to `Command[0x25]`, 37 bytes past `Command[0]`, with no check that `PIF_RAM` actually had that much room left from the command's start offset. Fixed: skip the mempak access (report absent, same as no controller) when it wouldn't fit.

Neither lead has been re-run against the CIC-6105 hang sweep yet -- same multi-hour soak-test caveat as lead #1 below.

**Correction: this hang is NOT rare and does not need a multi-hour soak to catch.** User report ("froze again, nothing works, video stopped") traced to clicking Majora's Mask in the ROM list. Reproduced twice more with all three leads above applied: once via the same UI click path (list screen hung mid-frame, ~108% CPU of one core, pixel-identical frame for 5+ seconds), once via `autoboot_rom=sd:/wii64/roms/Legend of Zelda, The - Majora's Mask (E) (M4) [!].z64` from a cold boot -- hangs at the very first "Starting ROM" debug screen, before any gameplay frame ever renders, every time tried. So the three fixes above did not resolve this ROM's hang; if anything it's now clear this was never a rare timing-dependent race, just a boot-time hang specific to (at least) this CIC-6105 title.

Profiled with `-DPERF_PROF` (`.dev/build_profiling.sh Rice_wii`) during the hung boot: `cpu: exceptions=N` climbs at roughly **1000/tick** the whole time it's stuck (`perfProf_cpuSample()`'s ~500ms cadence), vs. Banjo-Kazooie's ~200/tick during normal gameplay measured the same way, same build -- about 5x higher. `vis: 50.0`/vi-rate readings keep ticking on schedule throughout (that hook fires off a host-side VI timer, not guest progress, so it does NOT mean frames are actually advancing -- confirmed separately via screenshot, same frame throughout). So the guest CPU is not deadlocked (Dolphin still reports the process responding, CPU is genuinely busy), it's burning cycles on an elevated, sustained rate of guest exceptions without ever reaching a point that produces a new displayed frame -- consistent with (but not proof of) an exception-handling loop that keeps re-entering instead of completing boot.

**Pinned down further with `dynarec_trace=1` (`main/dynarec_trace.c`, the tool built for exactly this kind of hang -- draws the sampled dispatch PC straight to screen, so it survives even a hang Dolphin reports as unresponsive):** the freeze is fully deterministic. Every run stops at **dispatch #768, PC=`0x8009552C`**, immediately after dispatch #754 hit `PC=0x80000180` (the general exception vector) and returned normally -- 14 more ordinary dispatches followed before the freeze. Two follow-up screenshots ~10-20s apart showed byte-identical trace text, confirming dispatch count genuinely stops advancing (not just slow), while the exception counter keeps climbing the whole time. Since `dynarecTrace_dispatch()`/the `DYNAREC_WATCHDOG_CYCLE_LIMIT` safety net in `Wrappers.c` both only run from the dynarec's outer `while(!r4300.stop)` loop, and dispatch count is frozen while exceptions keep firing, whatever's looping at/after `0x8009552C` is calling into exception handling *without* returning to that outer loop -- a different, more evasive failure shape than the old "PC oscillates between 0 and 0x800023A0" bug the existing watchdog was built to catch (that one DID cycle through the outer loop repeatedly; this one appears not to reach it at all once stuck). **This means the existing watchdog cannot recover from this specific hang either** -- worth keeping in mind since it was built and tested against a different symptom.

Tested one concrete hypothesis against this exact repro (see `doc/reference-projects.md` for the full comparison): `gc_memory/pif.c`'s `cic_challenge` sticky flag (`update_pif_read()` silently no-op'd forever once set by certain PIF writes, instead of checking `PIF_RAMb[0x3F]`'s current value like Project64/mupen64plus-core both do) looked like a strong match for "guest spins waiting on PIF data our emulation never delivers." **Fixed it (commit `2492df2`) -- Banjo-Kazooie unaffected, but the Majora's Mask trace is byte-identical before and after, same dispatch #768/PC `0x8009552C` freeze point.** So that bug is real (kept, matches two mature reference implementations) but isn't what's stalling this ROM. Next step: figure out what MIPS instruction(s) the block at `0x8009552C` actually compiles from (needs a disassembly of the ROM around that address, or dynarec_trace extended to log the *translated* PPC around the freeze, not just the MIPS dispatch PC) and why its exception handling doesn't return control to the C-level dispatcher -- this is real dynarec-internals work, not a quick fix, and deserves its own dedicated session.
...
- `gc_memory/dma.c:123-137,199-213` (`dma_pi_read`/`dma_pi_write`) -- the SRAM/FlashRAM branch uses `pi_dram_addr_reg` completely unmasked as an `rdramb[]` index, and a length up to ~16MB as the `memcpy` size into/from a fixed 32KB `sram[]` buffer, no clamp at all. The Cart-ROM DMA branch in the same functions explicitly clamps against `rom_length`/`MEM_SIZE`/`MEMMASK` a few lines below -- the SRAM/FlashRAM branch never got the equivalent hardening. **APPLIED**: same clamping idiom as the Cart-ROM branch. Also found and fixed in passing: the sram-side mask (`&0xFFFE`, up to 0xFFFE) itself overran the real 32KB `sram[]` buffer (`saveSram()`/`loadSram()` size it at exactly 0x8000) -- fixed to `&0x7FFE`. Verified via the profiling build's `test_selectload` hook: a real ROM loads and correctly reports "Found & loaded save from SD card", exercising this exact path.
- `gc_memory/flashram.c:137-151` -- `erase_offset` (`(command & 0xffff) * 128`, up to ~8MB) indexes the fixed 128KB `flashram[]` buffer with no bound check. In the MEM2 layout that buffer is immediately followed by SRAM, MEMPACK, and the 4MB dynarec BLOCKS/RECOMPMETA regions -- an out-of-range offset can write straight into those. **APPLIED**: masked to `flashram[]`'s actual size at both of its two assignment sites.
- `gc_memory/flashram.c:178-211` (`dma_read_flashram`/`dma_write_flashram`) -- same unmasked-`pi_dram_addr_reg` pattern as the SRAM DMA above, no `& MEMMASK` anywhere. **APPLIED**: same `MEMMASK` clamp, plus a length clamp against both `flashram[]` and `rdram[]` bounds in `READ_MODE` (the one case that copies across a guest-controlled length in both directions at once).
- **APPLIED**: closed the systemic no-bounds-check gap across `memory.c`'s MMIO dispatch (`read_vi`/`write_vi`/`write_vid`, `read_ai`/`write_ai`/`write_aid`, `read_ri`/`write_ri`, `read_si`*, `read_rsp`/`write_rsp`(`_reg`), `read_dp`/`write_dp`, `read_dps`/`write_dps` -- ~10 function families indexing their backing pointer table by raw low-16-bits with no check in the fallback path). Also fixed the two blocks that DID bound-check but ~4x too permissively (`read_rdramreg`*, `read_pi`/`write_pi`*: compared the offset against the array's `sizeof()` instead of its element count). Verified: 129 `-Wsign-compare` warnings unchanged (the documented baseline, confirming no new warnings), 55+ seconds of real Banjo-Kazooie gameplay across 3 checkpoints all showing correct progression.
- Found in passing while extending this fix: `write_rsp_regd()` writes the high word of a dword store, then writes the low word to **the same address** again instead of `+4` -- every sibling dword-write function (`write_rid`, `write_aid`, `write_dpd`, the now-fixed `write_pifd`) correctly uses `[addr]` then `[addr+4]`. Looks like the same class of bug, but fixing it (verified building cleanly) produced a real regression -- a real ROM booted to a blank white screen instead of rendering. Reverted; left as a documented, deliberately-unfixed comment at the call site. Whatever `[*address_low+4]` normally holds here, something depends on it staying untouched -- needs real investigation (what register is actually at that offset, and why) before touching again, not a second guess.
- `gc_memory/pif.c:403-437` (`internal_ControllerCommand`, case 0x03) -- `case PLUGIN_RAW:`/`default:` are nested inside the mempak-range-check's `else` block rather than siblings of `case PLUGIN_MEMPAK:` (unlike the structurally identical case 0x02 block above it). Works today (valid C switch-label jump target), but a landmine for a future edit that doesn't realize this. Not touched -- pure restructuring with no behavior change intended, lower priority than the real bounds fixes above; left for a future pass.
...
**Fixes**:
- `r4300/ppc/Wrappers.c:472-582` (`dyna_mem`) -- every *load* case breaks its loop when `address` reads back 0 (the `write_nomem`/`read_nomem` sentinel set on a failed translation), but none of the *store* cases (`MEM_SW`/`SH`/`SB`/`SD`/`SWC1`/`SDC1`/`SWL`/`SWR`/`SDL`/`SDR`) check this -- a multi-word store that TLB-faults partway through keeps issuing further stores after the exception was already signaled and PC redirected. Doesn't leak a bad address, but a real store-after-exception correctness bug in exactly the MMIO/exception-interaction path this investigation cares about. **APPLIED**: same `if(!address) break;` check added to all ten store cases.
- `r4300/exception.h:33-38` -- `address_error_exception`, `TLB_invalid_exception`, `TLB_mod_exception`, `integer_overflow_exception`, `coprocessor_unusable_exception` are declared but **never defined or called anywhere**. Only `TLB_refill_exception`/`exception_general` exist. Concretely: misaligned accesses and integer overflow are never trapped -- `dyna_mem` never validates alignment, ADD/ADDI/SUB never check signed overflow. A misaligned/overflowing value that real hardware would trap immediately (keeping PC safely in the vector) instead flows through as "valid" here. Not proven to cause the CIC-6105 hang, but a real, verifiable gap in "is every abnormal-PC condition actually caught." **Deliberately not touched this pass**: implementing real trap logic for five different exception classes, threaded through the dynarec's hot ADD/ADDI/SUB and every memory access, is a correctness-critical rewrite that needs its own dedicated pass with real test coverage across several ROMs -- not something to bundle into a broad cleanup sweep.
- `r4300/Recomp-Cache-Heap.c:246-256` (`RecompCache_Alloc`) -- `node_heap` allocation retries `release(size)` exactly once, unlike every other allocation in the file (`cache`, `MetaCache_Alloc`) which loop until success. If one `release()` can't free enough space, `newBlock` stays NULL and the next line writes through it -- NULL-pointer write. Should match the retry-loop pattern used elsewhere in the same file. **APPLIED.**
- `r4300/interupt.c:333-347` (`gen_interupt`) -- dereferences `q->type` with no NULL check, unlike `check_interupt()`/`get_event()`/`remove_event()` in the same file which all guard `q == NULL`. Not known reachable today (persistent queue entries should always exist), but the one ungated spot, and `gen_interupt()` runs on every dynarec dispatch when `cp0_cycle_count >= 0` -- worth hardening defensively. **APPLIED.**
- **New, added this pass**: `r4300/r4300.h` -- the structural fragility named below ("Closest thing to a lead") had no compile-time guard. **APPLIED**: a `static_assert`/`_Static_assert` (guarded for both C and C++ translation units) pinning `local_gpr`'s offset relative to `gpr`, so a future field insertion/reordering near this spot becomes a build error instead of silently redirecting every JR/JALR to a garbage jump target. Verified: build passes (confirming today's layout still satisfies it), and a real, extended Banjo-Kazooie play session (well into its opening cutscene, 60 VI/s) ran correctly afterward.
- **New, this pass -- systemic**: the same single-retry allocation bug already found and fixed in `RecompCache_Alloc` (one `release(1)` call, no loop, `NULL` writes through if it can't free enough) recurs at 5 more sites: `Recompile.c`'s `recompile_block()` (the `func`/`hole`/`freed` allocations, 4 sites) and `init_block()` (3 `PowerPC_block` sites), plus `Wrappers.c`'s `dynarec()` (1 `PowerPC_block` site). **APPLIED** to all 6: same retry-until-success loop, since `release()` frees `PowerPC_func`/`PowerPC_block` structs back through plain `free()`, the same general heap every one of these `malloc`/`calloc` calls draws from. Verified: both plugins build clean, and a real, extended Banjo-Kazooie session (well past the opening cutscene) ran correctly, exercising the recompile/cache-eviction path this touches directly.
- **Correction to the "linked-block native loop" theory below, from a follow-up session**: the previous pass proposed `RecompCache_Link()` (patches a block's exit into a direct native branch, bypassing `dynarec()`'s C loop -- see `genJumpTo()`, `MIPS-to-PPC.c:4140-4168`) as the mechanism letting the Majora's Mask hang evade `dynarec_trace` and the cycle-limit watchdog, based on `dynarec_trace` showing a frozen dispatch count (#768) while the exception count kept climbing. **Direct instrumentation disproves this.** Adding a raw counter (`dynarec_dispatch_count`, incremented unconditionally every outer-loop iteration, not gated behind `dynarec_trace=1`) and comparing it against the exception count live shows the outer C dispatch loop is NOT frozen: by the time `exception_general()` had been called ~20,000 times, the outer loop had already dispatched **over 1,000,000** blocks; by ~114,000 exception calls, **over 5,700,000** dispatches. The C dispatcher is genuinely, continuously running -- nothing is bypassing it. The earlier "frozen at #768" reading was almost certainly an artifact of `dynarec_trace`'s own drawing, not the dispatch loop itself: `main/dynarec_trace.c`'s own header comment already warns that `wii64_debugBreadcrumb`'s raw GX calls are "only safe during the cold boot window before the graphics plugin's own renderer has touched it," and this hang runs long past that window. The trace tool can go stale (silently stop updating the screen) while dispatch continues -- exactly what its own code comments already flagged as a risk, just not one this investigation had hit before.
  - **What's actually true**: this ROM dispatches millions of blocks -- real, varied guest execution, not a frozen or trivially-cycling loop -- without ever completing boot or producing a visible frame, across every run tried (both sessions, no exception). The existing `DYNAREC_WATCHDOG_CYCLE_LIMIT` (300,000, ring size 16) never fires either, meaning whatever address pattern this is doesn't repeat within any 16-entry window across millions of dispatches -- either a working set much larger than 16, or (less likely given millions of dispatches with zero apparent forward progress) a genuinely unbounded pattern.
  - **Ideas tried and deliberately reverted this session, not landed**: (1) a second escape hatch in `exception_general()` using `setjmp`/`longjmp` to unwind out of a stuck loop from any C call site, gated by PC-cycle detection -- abandoned once the dispatch-count instrumentation showed there's no bypass to escape from; (2) the same escape hatch redesigned around dispatch-count stalling instead of PC cycles -- same fate, since the dispatch count was never actually stalling either. Both were cleanly reverted (`git checkout`) rather than left half-working; the setjmp/longjmp mechanism itself was never proven safe against this codebase's dynarec internals and shouldn't be resurrected without a confirmed use for it.
...
**Cleanup**:
- `LAZY_GEN_CALLS` machinery in `MIPS-to-PPC.c`/`Recompile.c`/`MIPS-to-PPC.h` -- never defined anywhere, its own comment admitted "hasn't proven to be faster". **APPLIED** (deleted; verified both plugins build clean from scratch and real gameplay still works).
- `r4300/compare_core.c:79-111` -- gated behind `COMPARE_CORE`, not defined anywhere, currently dead. Unlike `LAZY_GEN_CALLS`, `COMPARE_CORE` also gates code in `Wrappers.c`/`Wrappers.h`/`r4300.c`, a debug-comparison subsystem rather than an isolated unused code path -- deleting the whole thing is a bigger, riskier edit than this pass's other dead-code deletions, left alone as before. The `fread`/`fwrite` size-argument mismatch flagged previously (`:88,92,96`, swapped `size`/`count`, self-consistent between reader and writer so not an active bug, just confusing and wasteful) **APPLIED**: fixed to the idiomatic `fread(ptr, sizeof(element), count, f)` order on both the read and write side, zero build risk since the whole function is unreachable today.
- `r4300/profile.c` (`refresh_stat`, gated behind `PROFILE`, never defined) -- `IDLE_SECTION` (tracked via `start_section`/`end_section` in `main/timers.c`) had no corresponding print line, and `gui/DEBUG.h` already reserves `DBG_PROFILE_IDLE` right where it belongs (between `DBG_PROFILE_AUDIO` and `DBG_PROFILE_TLB`) -- clearly meant to be there, just dropped. **APPLIED**: added the missing line. Noted but not touched: the trailing "unknown" summary line uses a raw `NUM_SECTIONS+1` index that collides with `DBG_PROFILE_FP`'s slot -- both fight over the same on-screen row. Pure cosmetic collision in a debug overlay nothing builds by default; not worth restructuring the display-slot allocation for.
- `r4300/interupt.h` -- `compare_interupt()` and `gen_dp()` declared, never defined or called anywhere (repo-wide grep confirmed). **APPLIED** (removed).
- `r4300/r4300.h` -- stale commented-out `//extern char invalid_code[0x100000];`, superseded by `Invalid_Code.h`'s smaller pointer-based version (see that file's own header comment: "Uses 1/8th the memory"). **APPLIED** (removed).
- `r4300/ppc/Register-Cache.c` -- `#include <string.h>` appeared twice. **APPLIED** (removed the duplicate).
- `r4300/r4300.c` -- found and verified, not a bug: `cpu_init()`'s CIC-chip-5 (CIC-NUS-6105) branch writes 8 hardcoded boot-patch instruction words into `SP_IMEM`. Confirmed this runs *after*, not before, `init_memory()`'s `SP_IMEM` zeroing (`init_memory()` at ROM-load time, `cpu_init()` called later in the same load sequence -- `main/main_gc-menu2.cpp`'s `loadROM()`), so the patch survives. The specific instruction words themselves are unverified against a trusted reference (a genuine "is this the right patch" question, not this pass's finding) -- worth a dedicated check against Not64/Project64/mupen64plus-core if the CIC-6105 investigation above doesn't pan out.
