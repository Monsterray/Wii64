...
// x86 asm variant removed -- dead under NO_ASM (always defined, see
// Makefile.base). This C version was already what compiled here, per the
// comment it replaced ("use C code in other cases, this is probably faster
// anyway").
...
void InitRenderBase()
{
    // Always NoSSE -- isSSEEnabled is unconditionally false on PowerPC (see
    // isSSESupported() in Config.cpp), and the SSE path this picked between
    // was dead under NO_ASM (always defined here) regardless.
