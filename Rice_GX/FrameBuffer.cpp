...
uint32 CalculateRDRAMCRC(void *pPhysicalAddress, uint32 left, uint32 top, uint32 width, uint32 height, uint32 size, uint32 pitchInBytes )
{
    ...
    else
    {
        try{
            ...
            // NO_ASM is always defined for this project's builds (see
            // Makefile.base's CXXFLAGS); the x86 inline-asm variants that
            // used to follow here (__INTEL_COMPILER / __GNUC__+__x86_64__ /
            // plain x86 PIC and non-PIC) can never be selected and have
            // been removed.
