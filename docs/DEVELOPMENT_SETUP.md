...
3. Install the Wii toolchain and the libogc2 packages:

   ```sh
   sudo dkp-pacman -Syu
   sudo dkp-pacman -S wii-dev libogc2
   ```
...
command -v elf2dol
test -f "$DEVKITPRO/libogc2/wii/include/ogc.h"
...
The repository is on local branch `reconnaissance`, based on Wii64 `9eb19ee…`, with `origin` set to `git@github.com:Monsterray/Wii64.git`. Dolphin is installed. The devkitPro installer has been downloaded and checksum-verified, but the system package still requires one interactive macOS administrator authorization before `wii-dev` and `libogc2` can be installed.
